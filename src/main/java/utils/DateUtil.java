...
public final class DateUtil {
    ...
    public static Date parseDate(String source) throws Exception {
        synchronized (DATE_FORMAT) {
            try {
                ...
                throw new Exception("期待 yyyy-MM-dd 格式的日期,但输入格式为: " + source, e);
            }
        }
    }
     ...
     */
    public static Date parseDateMinute(String source) throws Exception {
        synchronized (DATE_MINUTE_FORMAT) {
            try {
                return DATE_MINUTE_FORMAT.parse(source);
            } catch (ParseException e) {
                throw new Exception("期待 yyyy-MM-dd HH:mm格式的日期,但输入格式为: " + source, e);
            }
        }
    }
